package pers.fsh.deviceutils.hardware.vectorhardware.mapping;

import com.sun.jna.Structure;

import java.util.Arrays;
import java.util.List;

/**
 * <i>native declaration : vxlapi.h</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class s_xl_eth_dataframe_tx_event extends Structure {
    /**
     * FPGA internal identifier unique to every frame sent
     */
    public int frameIdentifier;
    /**
     * Flags (see XL_ETH_DATAFRAME_FLAGS_)
     */
    public int flags;
    /**
     * Overall data length of <frameData>
     */
    public short dataLen;
    /**
     * currently reserved field - not used, ignore
     */
    public short reserved;
    /**
     * transmit duration of the Ethernet frame, in nanoseconds
     */
    public int frameDuration;
    /**
     * currently reserved field - not used, ignore<br>
     * C type : unsigned int[2]
     */
    public int[] reserved2 = new int[2];
    /**
     * Frame Check Sum
     */
    public int fcs;
    /**
     * Destination MAC address<br>
     * C type : unsigned char[6]
     */
    public byte[] destMAC = new byte[6];
    /**
     * Source MAC address<br>
     * C type : unsigned char[6]
     */
    public byte[] sourceMAC = new byte[6];
    /**
     * C type : T_XL_ETH_FRAMEDATA
     */
    public s_xl_eth_framedata frameData;

    public s_xl_eth_dataframe_tx_event() {
        super(1);
    }

    protected List<String> getFieldOrder() {
        return Arrays.asList("frameIdentifier", "flags", "dataLen", "reserved", "frameDuration", "reserved2", "fcs", "destMAC", "sourceMAC", "frameData");
    }

    public static class ByReference extends s_xl_eth_dataframe_tx_event implements Structure.ByReference {

    }

    ;

    public static class ByValue extends s_xl_eth_dataframe_tx_event implements Structure.ByValue {

    }

    ;
}
